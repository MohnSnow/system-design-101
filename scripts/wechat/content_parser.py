...
扫描 data/0.redbook_published/ 目录，配对 Markdown 和图片文件，
排除已归档到 wechat_published/ 的内容，解析 Markdown 内容。
"""
...
from scripts.redbook.models import ContentPair
...
class WechatContentParser:
    """微信内容解析器：扫描源目录、排除已归档内容、配对文件、解析 Markdown"""
    ...
    def scan(self) -> list[ContentPair]:
        """扫描源目录下所有 Markdown 文件（不递归子目录），排除归档目录中已存在同名文件的内容，
        返回按文件名字母顺序排序的 ContentPair 列表。
        """
        if not self.source_dir.exists():
            print(f"⚠️ 源目录不存在: {self.source_dir}")
            return []

        archived_stems = self._get_archived_stems()

        # 只扫描 source_dir 直接子文件，不递归子目录
        md_files = sorted(
        ...
        if not md_files:
            print("📭 源目录中没有 Markdown 文件")
            return []

        results: list[ContentPair] = []
        for md_path in md_files:
            # 排除已归档的文件
            if md_path.stem in archived_stems:
                continue

            image_path = self._find_image(md_path)
            if image_path is None:
                print(f"⚠️ 跳过 {md_path.name}：未找到同名图片文件")
                continue
    ...
    def _get_archived_stems(self) -> set[str]:
        """获取归档目录中所有 Markdown 文件的 stem（不含扩展名）。"""
        if not self.archive_dir.exists():
            return set()
        return {f.stem for f in self.archive_dir.iterdir() if f.is_file() and f.suffix == ".md"}
